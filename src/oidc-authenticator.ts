// src/oidc-authenticator.ts
//
// Verifies OIDC access tokens and maps each verified identity to an
// active user in this application's database.
...
// Errors caused by the token itself. Anyone can send a token that
// triggers these without holding a signing key, so they are bad
// credentials (401), not an authentication outage (503).
//
// Anything not listed, such as a failed key set download, is treated
...
const rejectedCredentialCodes = new Set([
  ...
  // An unrecognised "crit" header, or another token feature this
  // library does not support.
  "ERR_JOSE_NOT_SUPPORTED",

  // A token with no key ID while the provider publishes several keys,
  // which is normal during key rotation.
